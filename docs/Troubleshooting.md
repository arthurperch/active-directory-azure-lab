...
**Solution**:
1. In Azure Portal, confirm VM status is `Running`.
2. Check `nsg-ad-lab` inbound security rules. Ensure TCP 3389 allows your public IP.
...
**Cause**: Client VM pointing to public DNS or wrong static IP on domain controller.  
**Solution**:
1. On `client01`, run `Get-DnsClientServerAddress` to confirm the DNS server is `10.0.0.4`.
2. If not, set custom DNS in the NIC settings and restart the VM.
3. On `dc01`, confirm the NIC uses static IP `10.0.0.4`.
4. Flush caches: `ipconfig /flushdns` on client and server.
5. Retry `Resolve-DnsName dc01.mydomain.com`.
...
2. Ping `10.0.0.4` from `client01` to confirm network reachability.
3. Ensure NSG allows VirtualNetwork-to-VirtualNetwork traffic.
...
5. Verify the domain administrator password on `dc01` before returning to the client.
...
**Solution**:
1. On `dc01`, run `Import-Module ActiveDirectory` to confirm module availability.
...
**Solution**:
1. On `client01`, run `gpupdate /force` and wait for completion.
...
**Solution**:
1. Verify VM status is `Stopped (deallocated)`.
2. Check for remaining disks or snapshots in the resource group.
...
- Keep an admin PowerShell window open on `dc01` for quick checks (`dcdiag`, `Get-ADUser`, `Get-DnsServerResourceRecord`).
